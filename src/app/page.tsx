      ...
      <nav style={{background: '#ffffff', borderBottom: '1px solid rgba(135,206,235,0.4)', boxShadow: '0 2px 20px rgba(135,206,235,0.15)'}} className="sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <Link href="/search" className="text-sm font-medium hidden sm:block" style={{color: '#1E90FF'}}>
              Search
            </Link>
          ...
          <div className="max-w-md mx-auto mb-8">
            <div className="flex gap-2">
              ...
              <Link
                ...
                className="px-6 py-3 rounded-full font-bold text-white transition-all hover:scale-105 sky-btn"
              >
                🔍
              </Link>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {categories.map((cat) => (
            <Link key={cat.id} href={`/shop?category=${cat.slug}`} className="card p-5 text-center cursor-pointer group">
              ...
              <p className="text-sm font-bold group-hover:text-blue-500 transition-colors" style={{color: '#1E90FF'}}>{cat.name}</p>
            </Link>
          ))}
        </div>
      </div>
                ...
                <div key={product.id} className="bg-white rounded-lg overflow-hidden" style={{border: '1px solid #e8e8e8', boxShadow: '0 2px 8px rgba(0,0,0,0.06)'}}>
                  <Link href={`/product/${product.id}`}>
                    <div className="relative" style={{background: '#f9f9f9', height: '180px'}}>
                      ...
                      {discount && (
                        <div className="absolute top-2 left-2 px-2 py-0.5 rounded text-xs font-black text-white" style={{background: '#ff4444'}}>-{discount}%</div>
                      )}
                    </div>
                  </Link>
                  <div className="p-3">
                    ...
                    {discount && <p className="text-xs font-bold mb-2" style={{color: '#ff4444'}}>-{discount}% OFF</p>}
